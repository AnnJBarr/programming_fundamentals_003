const catalogueService = require("../app/catalogue_service");

describe("catalogueService", () => {
  describe("catalogueService.checkBook", () => {
    test("returns true when a book is found by title", () => {
      expect(catalogueService.checkBook("Great Expectations")).toBe(true);
    });

    test("returns false when a book cannot be found by title", () => {
      expect(catalogueService.checkBook("Gone With The Wind")).toBe(false);
    });

    test("returns true when a partial match is found", () => {
      expect(catalogueService.checkBook("Expectations")).toBe(true);
    });

    test("returns false when not even a partial match is found", () => {
      expect(catalogueService.checkBook("The Wind")).toBe(false);
    });
  });

  describe("catalogueService.countBooksByKeyword", () => {
    test("returns 3 when title contains assassin", () => {
      expect(catalogueService.countBooksByKeyword("assassin")).toBe(3);
    });

    test("returns 2 when title contains normal", () => {
      expect(catalogueService.countBooksByKeyword("normal")).toBe(2);
    });

    test("returns 0 when title does not contain pineapple", () => {
      expect(catalogueService.countBooksByKeyword("pineapple")).toBe(0);
    });

    test("returns 1 when title does contains 6", () => {
      expect(catalogueService.countBooksByKeyword("6")).toBe(1);
    });

  })

  describe("catalogueService.getBooksByAuthor", () => {
    test("returns A Tale of Two Cities, Oliver Twist, Great Expectations when passed Charles Dickens", () => {
      expect(catalogueService.getBooksByAuthor("Charles Dickens")).toEqual([
        "A Tale of Two Cities",
        "Oliver Twist",
        "Great Expectations"
      ]);
    });

    test("returns The Blind Assassin when passed Margaret Atwood", () => {
      expect(catalogueService.getBooksByAuthor("Margaret Atwood")).toEqual([
        "The Blind Assassin"
      ]);
    });

    test("returns a blank array when passed Kenneth Graham", () => {
      expect(catalogueService.getBooksByAuthor("Kenneth Graham")).toEqual([
      ]);
    });

    test("returns A Tale of Two Cities, Oliver Twist, Great Expectations, The Origin of Species when passed Charles", () => {
      expect(catalogueService.getBooksByAuthor("Charles")).toEqual([
        "A Tale of Two Cities",
        "Oliver Twist",
        "Great Expectations",
        "The Origin of Species"
      ]);
    });
  });

  describe("catalogueService.getStockCount", () => {
    test("returns 9 when passed Between the Assassinations", () => {
      expect(catalogueService.getStockCount("Between the Assassinations")).toEqual(9);
    });

    test("returns 3 when passed A Tale of Two Cities", () => {
      expect(catalogueService.getStockCount("A Tale of Two Cities")).toEqual(3);
    });

    test("returns 0 when passed Dracula", () => {
      expect(catalogueService.getStockCount("Dracula")).toEqual(0);
    });

    test("returns Not in our catalogue when passed Gone with the Wind", () => {
      expect(catalogueService.getStockCount("Gone with the Wind")).toEqual("Not in our catalogue");
    });
  });

  describe("catalogueService.stockReview", () => {
    test("returns Not in Stock when passed Dracula", () => {
      expect(catalogueService.stockReview("Dracula")).toEqual("Not in Stock");
    });

    test("returns Medium Stock when passed The Blind Assassin", () => {
      expect(catalogueService.stockReview("The Blind Assassin")).toEqual("Medium Stock");
    });

    test("returns Not in our catalogue when passed Gone with the Wind", () => {
      expect(catalogueService.stockReview("Gone with the Wind")).toEqual("Not in our catalogue");
    });

    test("returns Low Stock when passed A Tale of Two Cities", () => {
      expect(catalogueService.stockReview("A Tale of Two Cities")).toEqual("Low Stock");
    });

    test("returns High Stock when passed The Origin of Species", () => {
      expect(catalogueService.stockReview("The Origin of Species")).toEqual("High Stock");
    });

  });

    
})
